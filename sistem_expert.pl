...
scopuri_princ.	
*/
create_dir :- if((directory_exists('output_benzi_desenate')), 
				 (delete_directory('output_benzi_desenate', [if_nonempty(delete)]), make_directory('output_benzi_desenate')),
...
pot_interoga(Stream,av(Atr,_),Istorie) :-
	not interogat(av(Atr,_)),
	interogabil(Atr,Optiuni,Mesaj),
	write('\n interogabil optiuni: '), write(Optiuni), write(', atribut: '), write(Atr),
...
interogheaza(Stream,Atr,Mesaj,Optiuni,Istorie) :-
	write(Stream,i(Mesaj)),nl(Stream),flush_output(Stream),
	write('\n optiunii: '), write(Optiuni),
	append(Optiuni, [nu_stiu, nu_conteaza], OptiuniNoi),
	citeste_opt(Stream,VLista,OptiuniNoi,Istorie),
	assert_fapt(Atr,VLista).
%%%	
...
trad(interogabil(Atr,M,P)) --> 
	[intreaba,'@',Atr],afiseaza(Atr,P),lista_optiuni(M).
...
atunci(Atunci,FC) --> [concluzie, '@', '('], propoz(Atunci),[',', fc, '@', FC, '@', ')'].
atunci(Atunci,100) --> [concluzie, '@', '('], propoz(Atunci), [')'].
...
caracter_cuvant(C):-member(C,[44,59,58,63,33,46,41,40,64,124]).
...
inceput:-format('Bun venit la primul nostru Sistem expert\n',[]),	flush_output,
